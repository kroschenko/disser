...
{   
 \inserttitlegraphic
  ...
  \begin{block}{}
    \centering
    \textbf{КРОЩЕНКО} Александр Александрович,\\ старший преподаватель кафедры ИИТ, к.т.н.
  \end{block}
  \vspace*{3em}
...
\title[]{\textbf{Системы компьютерного зрения на базе предобученных НС}}   

% \author[]{\textbf{КРОЩЕНКО} Александр Александрович,\\ старший преподаватель кафедры ИИТ, к.т.н.}
...
\begin{document}
	...
	\end{frame}
        ...
        %     \newline
        %     \begin{itemize}
        %         \item AlexNet -- 62.3 млн параметров (2012)
        %         \item VGG16 -- 138 млн параметров (2014)
        %         \item ResNet-50 -- 25.6 млн параметров (2015)
        %         \item NASNet -- от 22.6 до 138 млн параметров (2017)
        %         \item Stable Diffusion -- 890 млн параметров (2022)
        %         \item DALL-E 2 -- 3.5 млрд параметров (2022)
        %         \item EVA-02-CLIP -- 5 млрд параметров (2023)
        %     \end{itemize}
        %     % \begin{figure}[H]
        ...
        %     \begin{block}{Цель}
        %         \large
        %         Разработка \textbf{эффективных методов и алгоритмов} для обучения глубоких нейронных сетей, используемых для решения \textbf{задач компьютерного зрения}, включающих \textbf{обнаружение солнечных панелей} на аэрофотоснимках и \textbf{распознавание маркировки продукта} на конвейерной линии
        %     \end{block}
        ...
        \begin{frame}{Обучение с использованием неконтролируемого предобучения на основе RBM}
            \begin{columns}
                \begin{column}{0.5\textwidth}
                    ...
                    \end{figure}
                    \underline{Этапы обучения (Хинтон, 2006)}:
                    \begin{enumerate}
        ...
        %     \begin{block}{1}
        %         \large
        %         \justifying{\textbf{Установление эквивалентности} задач максимизации функции правдоподобия распределения входных данных, минимизации кросс-энтропийной функции ошибки и суммарной квадратичной ошибки при использовании линейных нейронов в пространстве синаптических связей ограниченной машины Больцмана, что позволяет учитывать нелинейную природу нейронных элементов.}
        %     \end{block}
        % \end{frame}
        ...
        \begin{frame}{Альтернативное представление RBM (1)}
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.8\textwidth]{figure_1_2.drawio.png}
            \end{figure}
            \begin{center}
                \begin{tabular}{c|c}
        ...
        %     \begin{block}{2}
        %         \large
        %         \justifying{\textbf{Метод обучения ограниченной машины Больцмана}, базирующийся на эквивалентности задач максимизации функции правдоподобия распределения входных данных и минимизации суммарной квадратичной ошибки при использовании линейных нейронов в пространстве синаптических связей сети, что позволяет \textbf{расширить класс обучаемых моделей и повысить обобщающую способность глубоких нейронных сетей}.}
        %     \end{block}
        % \end{frame}
        ...
        %     \newline
        %     \begin{itemize}
        %         \item \justifying{для полносвязных слоев \textit{N} x \textit{M}, где \textit{N} обозначает количество входных нейронов слоя, а \textit{M} -- количество выходных нейронов.}  
                                
        %         \item \justifying{для сверточных слоев \textit{K} x \textit{S} x \textit{S}, где \textit{K} обозначает количество ядер свертки в соответствующем слое, a \textit{S} x \textit{S} -- размерность ядра свертки.}
        %     \end{itemize}
        %     \justifying{При этом для полносвязных слоев используется сокращенная форма нотации, обозначающая количество нейронов в каждом слое, например, \textbf{784-100-100-10}, где 784 -- это число нейронов в первом (распределяющем) слое, 100 -- число нейронов во втором слое и так далее.} 
        % \end{frame}
        ...
        %         \item REBA -- обучение с предобучением (только предлагаемый метод); 
        ...
        %     При обучении с помощью гибридного подхода первый слой глубокой нейронной сети обучался как RBM с использованием классического метода обучения, а все последующие обучались предложенным методом.
        % \end{frame}
        ...
        %     \begin{block}{3}
        %         \large
        %         \justifying{\textbf{Алгоритм редуцирования параметров глубокой нейронной сети}, базирующийся на неконтролируемом предобучении сети, что позволяет упростить ее архитектуру (путем сокращения числа настраиваемых параметров модели) \textbf{без потери обобщающей способности}.}
        %     \end{block}
        % \end{frame}
        ...
        %     \begin{column}{0.6\textwidth}
        %     \begin{enumerate}
        %         \item \textbf{Неконтролируемое предобучение НС}, начиная с первого слоя;
        %         \item \textbf{Обнуление} параметров НС, значения которых попадают в интервал $[-t, t]$ для заданного $t>0$;
        %         \item \textbf{Архитектурная реконфигурация} НС.\\
        ...
        %     \begin{block}{4}
        %         \large
        %         \justifying{\textbf{Нейросетевая система компьютерного зрения}, основывающаяся на предлагаемом методе предобучения нейросетевых моделей, применение которой позволяет улучшить качество решения задач классификации.}
        %     \end{block}
        % \end{frame}

        \begin{frame}{Общая архитектура систем компьютерного зрения}
            \begin{figure}[H]
                \centering
        ...
        \begin{frame}{Обнаружение солнечных панелей на аэрофотоснимках}
            \small
            \begin{block}{Цель разработки}
                Осуществить детекцию солнечных панелей на фотоснимках, полученных из выборки, сформированной с использованием Google Maps
            \end{block}
        ...
        \begin{frame}{Схема алгоритма для обнаружения солнечных панелей}
            \begin{figure}
                \centering
                \includegraphics[width=0.36\textwidth]{solar_panels_detection.png}
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Применяемые модели}
            ...
            \begin{figure}
                \centering
                \includegraphics[width=1\textwidth]{Faster R-CNN.png}
            \end{figure}
        \end{frame}
        ...
        \begin{frame}{Результаты обучения и тестирования моделей (1)}
            \begin{columns}[T]
                ...
                \end{column}
            \end{columns}
            Применение предложенного метода предобучения позволило увеличить точность модели обнаружения панели на \textbf{7,31\%}.
        \end{frame}

        \begin{frame}{Распознавание маркировки продукта на производственной линии}
            \small
            \begin{block}{Цель разработки}
        ...
        \begin{frame}{Результаты обучения и тестирования моделей (2)}
            ...
            \begin{columns}[T]
                ...
                \end{column}
            \end{columns}
            Применение предлагаемого метода предобучения позволило увеличить точность модели классификатора положения продукта на \textbf{5,24\%}.
        \end{frame}
        ...
        %         \item \justifying{Головко, В.А. Об одном методе обучения нейронных сетей глубокого доверия / В.А. Головко, А.А. Крощенко // Вычислительные методы, модели и образовательные технологии : сб. матер. Междунар. науч.-практич. конф., Брест, 15-16 окт. 2014 г. / Брест. гос. ун-т им. А.С. Пушкина; под общ. ред. О.В. Матысика. --- Брест, 2014. --- С. 98--99.}
        %     \end{enumerate}
        % \end{frame}
        ...
        %         \item \justifying{Крощенко, А.А. Применение глубокой нейронной сети для решения задачи распознавания образов / А.А. Крощенко // Вычислительные методы, модели и образовательные технологии : сб. матер. Междунар. науч.-практич. конф., Брест, 21 окт. 2016 г. / Брест. гос. ун-т им. А.С. Пушкина; под общ. ред. О.В. Матысика. --- Брест, 2016. --- С. 132--133.}
        %     \end{enumerate}
        % \end{frame}
        ...
        %         \centering
        %         \includegraphics[width=4cm]{act_BSTU.jpg}
...
% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (1)}
%     \small
%     \underline{\textbf{Оппонент -- д.т.н., профессор Старовойтов Валерий Васильевич:}}
%     \begin{enumerate}
%         \item \justifying{Автором разработан один метод, а в названии указано множественное число.}
        
%         $\rightarrow$ \textit{\textbf{Частично согласен. Фактически действительно рассматривается один метод, применяемый для неконтролируемого предобучения глубоких нейронных сетей. Однако в работе приводятся стратегии применения данного метода, которые могут рассматриваться в контексте отдельных подходов (REBA и HREBA), также приводятся модифицикации предлагаемого метода для разных случаев полносвязных и сверточных слоев нейронных элементов.}}
%         \item \justifying{В работе используется параметр $\alpha$ -- скорость обучения. В работе нет объяснения почему она постоянна в формулах (2.7) и (2.8) и одинакова для x и y.}   
        
%         $\rightarrow$ \textit{\textbf{В данной работе, следуя хинтоновскому методу обучения, скорость обучения RBM действительно константна, в данном случае мы опираемся на оригинальные работы, где скорость обучения никак не варьируется. Однако, автором проводятся исследования, связанные с применением адаптивного шага обучения (например, V. Golovko, E. Mikhno, A. Kroschanka, M. Chodyka and P. Lichograj, <<Adaptive Learning Rate for Unsupervised Learning of Deep Neural Networks>> 2023 International Joint Conference on Neural Networks (IJCNN), Gold Coast, Australia, 2023, pp. 1--6)}}       
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (2)}
%     \small
%     \underline{\textbf{Оппонент -- д.т.н., профессор Старовойтов Валерий Васильевич:}}
%     \begin{enumerate}
%         \setcounter{enumi}{2}
%         \item \justifying{В разделе 3.2.1 автор вводит понятие <<эффективность классификации>> которое на самом деле является общепринятой точностью (accuracy), т.е. отношением числа диагональных элементов матрицы ошибок к сумме всех элементов. Эффективность обучения сети на базе CIFAR100, указанная в таблице 3.13 составляет от 2,67\% до 20,84\%. Это очень низкие показатели. Аналогично в таблицах 3.11-3.12 указана невысокая эффективность, в районе 45-58\%.}

%         $\rightarrow$ \textit{\textbf{С первой частью замечания согласен -- в работе действительно под эффективностью понимается точность классификации. В указанных экспериментах осуществлялось сравнее методов редуцирования параметров глубоких полносвязных нейронных сетей. Невысокие проценты распознавания для выборок CIFAR10 и CIFAR100 объяснимы применяемыми в данных экспериментах полносвязными НС, а точнее общим уровнем их эффективности для указанных задач.}}
%         \item \justifying{В главе 3 не указано на каком числе образцов ирисов Фишера выполнено обучение и на каком числе тестовых образцов получена одна ошибка классификации. В процентном отношении это может быть большое число.}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен. Для данной задачи мной производилось разбиение выборки в соотношении 4 к 1.}}%, но указанный результат для ошибки на одном образе рассчитывался относительно полного объема выборки.}}        
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (3)}
%     % \small
%     \underline{\textbf{Оппонент -- д.т.н., профессор Старовойтов Валерий Васильевич:}}
%     \begin{enumerate}
%         \setcounter{enumi}{4}
%         \item \justifying{В задаче контроля маркировки выполняется детекция печатных символов. На С.88, раздел 4.3.4 указано, что <<итоговая эффективность распознавания маркировки составила 93.27\%>>. Мое мнение -- это маловато для контроля качества. Не описано какие действия могут быть предприняты после распознавания неверной маркировки.}

%         $\rightarrow$ \textit{\textbf{Данный процент достаточен для выявления ошибочной маркировки из-за специфики обработки ошибок маркировки. После выявления ошибочной маркировки продукты с такой маркировкой отбраковываются, помимо этого необходимо остановить конвейер, поскольку все последующие продукты также будут содержать ошибки в маркировке.}}
%         \item \justifying{В разделе 4.2 описано решение задачи детекции солнечных батарей на аэрофотоснимках. Было бы полезно сравнить полученные соискателем результаты с результатами других исследователей, полученных на общедоступных базах подобных изображений, например, описанных в работе de Hoog J, et. al. Using satellite and aerial imagery for identification of solar pv: State of the art and research opportunities // Proc. 8th ACM Int. Conf. on Future Energy Systems, 2020, pp. 308-313.}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен}}
%         % \item В задаче контроля маркировки выполняется детекция печатных символов. На С.88, раздел 4.3.4 указано, что <<итоговая эффективность распознавания маркировки составила 93.27\%>>. Мое мнение -- это маловато для контроля качества. Не описано какие действия могут быть предприняты после распознавания неверной маркировки.
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (4)}
%     \underline{\textbf{Оппонент -- д.т.н., доцент Богуш Рихард Петрович:}}
%     \begin{enumerate}
%         \item \justifying{В главе 2 для теоремы 5, которую автор представил, как обобщающую теоремы 1--4 при установлении эквививалентности использования разных критериев при обучении нейронных сетей, отсутствует доказательство. Следовало бы привести доказательство, либо использовать другое утверждение.}
        
%         $\rightarrow$ \textit{\textbf{В теоремах 1--4 приводятся доказательства отдельных случаев. В теореме 5 в данном случае просто резюмируются результаты, полученные ранее.}}

%         \item \justifying{В главе 4 представлена структура системы распознавания маркировки продукта, предполагающая проверку валидности результата распознавания. Однако не сказано, как работает данный модуль, и каково его влияние на итоговую точность системы.}
        
%         $\rightarrow$ \textit{\textbf{Итоговая точность данного модуля связана с результатами, полученными нейросетевым модулем детекции составных частей маркировки, сам модуль в данном случае реализован тривиально -- путем сравнения результата распознавания с заранее заданным шаблоном.}}
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (5)}
%     \underline{\textbf{Оппонент -- д.т.н., доцент Богуш Рихард Петрович:}}
%     \begin{enumerate}
%         \setcounter{enumi}{2}
%         \item \justifying{Автор использует термин <<тонкая настройка>> параметров нейронной сети (стр. 24 и далее), т.е. прямой перевод от английского <<fine tuning>>. Считаю, что более корректно было бы использовать термин <<точная настройка>>.}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен}}

%         \item \justifying{Представление результатов экспериментов в главе 3 было бы более полным при наличии в диссертации графиков функций потерь при обучении нейронных сетей с использованием разработанных подходов и существующих, аналогично рис. 3.7 (стр. 60). Следует также отметить, что на данном рисунке обозначения в легенде раскрыты значительно позже, на стр. 63, что усложняет его понимание.}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен}}
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (6)}
%     \underline{\textbf{Эксперт -- к.т.н., доцент Перцев Дмитрий Юрьевич:}}
%     \begin{enumerate}
%         \item \justifying{Соискатель защищает алгоритм REBA, однако в предварительных тестах, представленных в работе, присутствует Hybrid REBA (с. 65), который дает более качественный результат. С учетом этого не ясно, почему акцент сделан именно на алгоритм REBA.}
        
%         $\rightarrow$ \textit{\textbf{REBA и HREBA определяют стратегию предобучения глубокой нейронной сети. В положения к защите вынесен метод обучения ограниченной машины Больцмана, но не стратегии предобучения ГНС. Применение предлагаемого метода для обоих стратегий однообразное.}}
%         \item \justifying{В приведенных таблицах на с. 64--67 указана <<Эффективность>> в процентах для результатов обучения. Из работы не ясно, что подразумевается под понятием <<эффективность>>.}
        
%         $\rightarrow$ \textit{\textbf{В работе под <<эффективностью>> понимается точность.}}
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (7)}
%     \underline{\textbf{Эксперт -- к.т.н., доцент Перцев Дмитрий Юрьевич:}}
%     \begin{enumerate}
%         \setcounter{enumi}{2}
%         \item \justifying{В работе не приводятся экспериментальные исследования классических методов редуцирования для сверточных нейронных сетей.}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен. В настоящий момент на стадии предпечатной подготовки находится статья, которая посвящена применению предложенного метода для редуцирования данного типа НС (PRIA'24 -- An Approach to Pruning the Structure of Convolutional Neural Networks Without Loss of Generalization Ability).}}
%         \item \justifying{В работе присутствуют формулы без отсылки на первоисточник (например, CEv и CEh на с. 41), некоторые математические выводы не очевидны (например, на с. 46 и 47).}
        
%         $\rightarrow$ \textit{\textbf{С замечанием согласен.}}
%         \item \justifying{По тексту присутствуют незначительные опечатки и описки (например, <<...критерию 2.16...>> (с. 37, формула 2.16 -- формула вычисления ошибки).}
    
%         $\rightarrow$ \textit{\textbf{С замечанием согласен.}}
%     \end{enumerate}
% \end{frame}

% \begin{frame}{Комментарии к замечаниям оппонентов и экспертов (8)}
%     \underline{\textbf{Эксперт -- д.ф.-м.н. Редько Владимир Георгиевич:}}
%     \justifying{В тексте автореферата не приведены правила обучения сверточной ограниченной машины Больцмана, хотя присутствуют результаты, полученные с использованием именно данного типа моделей. Также отсутствует описание типов предобучения, но есть упоминание предобучения І типа.}
%     \newline
%     \underline{\textbf{Эксперт -- д.т.н., профессор Азаров Илья Сергеевич:}}
%     \justifying{В тексте автореферата отсутствует подробное описание алгоритма редуцирования параметров нейросетевой модели, дается лишь краткое описание и результаты проведенных экспериментов. Кроме этого, в автореферате нет описания используемой соискателем нотации слоев нейронных сетей.}
%     \newline
%     \underline{\textbf{Эксперт -- к.ф.-м.н., доцент Будько Александр Евгеньевич:}}
%     \justifying{Не разъясняется, каким образом были получены правила обучения для предлагаемого метода.
%     Остается неясным, что собой представляет валидатор маркировки продукции и каким образом производится валидация маркировки.}
%     \newline
%     $\rightarrow$ \textit{\textbf{Со всеми замечаниями по автореферату согласен}}
% \end{frame}
